<?php
class form_validation extends common_function{
	protected $CI;
	...
	{  parent::__construct();
        ...
        public function get__error_cnt(){
            return count($this->_error_array);
        }
        
        public function get__error_array(){
            return $this->_error_array;
        }

        public function language_load($str = 'form_validation'){
            $load_lang_file_obj = new load_language_file();
            $load_lang_file_obj->settle_lang_message();
            $cls_lang_arr = $load_lang_file_obj->receive_lang_message();
            if(!empty($cls_lang_arr)){
                $cls_lang_arr = $cls_lang_arr[$str];
            }
            $this->CI = $cls_lang_arr;
        }
	public function set_rules($field, $label = '', $rules = array(), $cls_errors = array())
	{	if ($_SERVER["REQUEST_METHOD"] !== 'POST' && empty($this->validation_data))
		...
		if (is_array($field))
		{
			...
			{	if ( ! isset($row['field'], $row['rules']))
				...
				$cls_errors = (isset($row['errors']) && is_array($row['errors'])) ? $row['errors'] : array();
				$this->set_rules($row['field'], $label, $row['rules'], $cls_errors);
			}
	...
	public function set_message($lang, $val = '')
	{
        ...
        public function add_message($lang, $val = '')
	{
		if ( ! is_array($lang))
		{
			$lang = array($lang => $val);
	...
	{	if (count($this->_error_array) === 0)
		...
		$str = '';
                if(!empty($this->_error_array)){
                    $str = $prefix . implode($suffix . "\n" . $prefix, $this->_error_array) . $suffix;
        }
		return $str;
	}
	public function run($group = '')
	{
		...
		$this->language_load('form_validation');
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0){
		...
		foreach ($rules as $rule)
		{
			...
			if ($result === FALSE)
			{
				...
				$message = $this->_build_error_message($line, $this->_translate_fieldname($row['label']), $param);
	...
	{
            $line = $this->CI['form_validation_error_message_not_set'];
            if (isset($this->_field_data[$field]['errors'][$rule])){
                $line = $this->_field_data[$field]['errors'][$rule];
            }
            elseif (isset($this->_error_messages[$rule])){
                $line = $this->_error_messages[$rule]; 
            }
            elseif (isset($this->CI['form_validation_'.$rule])){
                $line = $this->CI['form_validation_'.$rule];
            }
            return $line;
	}
	...
	protected function _build_error_message($line, $field = '', $param = '')
	{
	...
	public function prep_for_form($row)
	{
		if ($this->_safe_form_data === FALSE OR empty($row))
		{
			return $row;
		}
		...
		if (is_array($row))
		{
			...
			{
				$row[$key] = $this->prep_for_form($val);
			}

			return $row;
		}

		return str_replace(array("'", '"', '<', '>'), array('&#39;', '&quot;', '&lt;', '&gt;'), stripslashes($row));
	}
	...
	public function reset_validation()
	{
        ...
        private function valid_color_hex($hex_value){
            return (bool) preg_match("/#([a-f0-9]{3}){1,2}\b/i", $hex_value);
        }
        private function is_float($number) {
